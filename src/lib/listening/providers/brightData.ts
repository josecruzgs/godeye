...
async function brightDataRequest(path: string, init?: RequestInit): Promise<Response> {
...
async function runPlatform(
  ...
  if (pending) {
    ...
    return collectSnapshot(platform, pending, since, until, snapshots, key);
  }
  ...
  return collectSnapshot(platform, snapshotId, since, until, snapshots, key);
}
...
async function collectSnapshot(
  ...
  while (Date.now() < deadline) {
    ...
    const progress = await brightDataRequest(`/progress/${snapshotId}`);
  ...
  const snapshot = await brightDataRequest(`/snapshot/${snapshotId}?format=json`);
